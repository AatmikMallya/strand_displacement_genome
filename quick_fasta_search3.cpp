...
bool are_compatible(char a, char b,bool include_wobble=false)
{
    a = toupper(a);
    b = toupper(b);
    if (a == 'U')
...
int find_longest_stretch(const string& seq, int &sA_start, int& sA_end, int& sB_start, int& sB_end, bool include_wobble=false)
{
 int N = seq.length();
...
int main(int argc, char **argv)
{
    ...
    try
    {
       ...
       for (auto i = rs.begin(); i != rs.end(); ++i)
       {
           ...
           cout << (*i).second.length() << " "  << maxlen << " " <<  get_xds((*i).second,maxlen) << " " << sAs << "  "  << sAe  << " " << sBs << "  "  << sBe <<  endl;
       }
    }
    catch(exception& e)
    {
